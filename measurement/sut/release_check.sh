...
find_paper_dir() {
  local root="$1"
  local pattern
  for pattern in "*Paper 2*" "*paper2*" "paper2-manuscript"; do
    local match
...
PAPER_DIR="${WORKSPACE_ROOT}/paper2-manuscript"
for search_root in "$WORKSPACE_ROOT" "$STICKS_ROOT"; do
  if found_paper_dir="$(find_paper_dir "$search_root")"; then
...
rule_label_map = {
    'R1_ID_PLATFORM_KEYWORD': r'Platform keyword \((?:\\texttt{Windows Domain}/identity/IaaS/SaaS|domain / identity / IaaS / SaaS)\)',
...
with open(meas_scripts / 'results' / 'audit' / 'compatibility_rule_breakdown.csv', newline='', encoding='utf-8') as f:
    for row in csv.DictReader(f):
        label_pattern = rule_label_map[row['rule_id']]
        if row['rule_id'] == 'R1_ID_PLATFORM_KEYWORD':
            latex_row_pattern = (
                rf"{label_pattern} & {re.escape(row['class'])} & {re.escape(row['count'])} & "
                rf"{float(row['pct_all_techniques']):.1f}\\% \\\\"
            )
        else:
            latex_row_pattern = (
                rf"{re.escape(label_pattern)} & {re.escape(row['class'])} & {re.escape(row['count'])} & "
                rf"{float(row['pct_all_techniques']):.1f}\\% \\\\"
            )
        if not re.search(latex_row_pattern, main):
            print(f"[release-check][FAIL] compatibility rule breakdown row missing/mismatched in main.tex: {latex_row_pattern}")
            sys.exit(1)

print('[release-check] static table checks OK')
PY
else
  log "3b) No manuscript tree detected; skipping static table checks"
fi
...
PY
  ...
  log "3c) No manuscript tree detected; skipping bibliography policy checks"
fi
...
[[ -f "$PAPER_DIR/figures/tier_collapse_template.tex" ]] || fail "missing tier_collapse_template.tex"
  ...
  log "4-6) No manuscript tree detected; skipping manuscript build and hygiene checks"
fi
...
if generated <= 0:
    ...
    sys.exit(1)
if os.environ.get("HAS_MANUSCRIPT_DIR") == "1" and used <= 0:
    print('[release-check][FAIL] macro coverage report indicates empty generated/used macro sets')
    sys.exit(1)
...
if [[ "$HAS_MANUSCRIPT_DIR" == "1" ]]; then
  log "PASS: pipeline + data + manuscript checks are consistent"
else
  log "PASS: pipeline + data + released output checks are consistent"
fi
