#!/usr/bin/env python3
"""
Stage a complete but sanitized public repository for the Paper 2 STICKS artifact.

This package is intentionally broader than the minimal review artifact:
it retains the canonical smoke path, the VM-backed lab helpers, the paper
sources, and the current evidence/results needed to reproduce the paper and
inspect the heavier realism path. Runtime VM images, local caches, and
developer-only secrets/configs are excluded.
"""
...
def stage_paper(dest_root: Path) -> None:
    ...
    for relative in PAPER_FILES:
        src = PAPER2_ROOT / relative
        if src.exists():
            copy_file(src, paper_dest / relative)
    if (PAPER2_ROOT / "figures").exists():
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Stage a complete sanitized public repo for the Paper 2 STICKS artifact."
    )
    parser.add_argument(
        "--dest",
