    ...
    for abl in abilities:
        try:
            ...
            if resp.status_code == 200:
                print(f"[OK] Restored ability: {abl.get('name')} ({abl.get('ability_id')})")
                success_count += 1
            else:
                print(f"[WARN] Failed to restore {abl.get('name')}: {resp.status_code} - {resp.text}")
        except Exception as e:
            print(f"[FAIL] Error restoring {abl.get('name')}: {e}")
