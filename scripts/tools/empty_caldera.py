...
def delete_operations():
    ...
    if resp.status_code != 200:
        print(f"[FAIL] Failed to fetch operations: {resp.status_code} - {resp.text}")
        return False
    
    operations = resp.json()
    if not operations:
        print("[WARN] No operations found.")
        return True
    ...
    for op in operations:
        ...
        if del_resp.status_code in (200, 204):
            print(f"[OK] Deleted {op_name} ({op_id})")
        else:
            print(f"[FAIL] Failed {op_name} ({op_id}): {del_resp.status_code} - {del_resp.text}")
            success = False
...
def delete_abilities():
    ...
    if resp.status_code != 200:
        print(f"[FAIL] Failed to fetch abilities: {resp.status_code} - {resp.text}")
        return False
    
    abilities = resp.json()
    if not abilities:
        print("[WARN] No abilities found.")
        return True
    ...
    for ability in abilities:
        ...
        if del_resp.status_code in (200, 204):
            print(f"[OK] Deleted {ability_id}")
        else:
            print(f"[FAIL] Failed {ability_id}: {del_resp.status_code} - {del_resp.text}")
            success = False
...
def delete_adversaries():
    ...
    if not adversaries:
        print("[WARN] No adversaries found.")
        return True
    ...
    for adv in adversaries:
        ...
        if del_resp.status_code in (200, 204):
            print(f"[OK] Deleted {adv_id}")
        else:
            print(f"[FAIL] Failed {adv_id}: {del_resp.status_code} - {del_resp.text}")
            success = False
...
def main():
    ...
    if all_success:
        ...
        print(f"\n[WARN]  Some deletions failed. Check the logs above.")
        sys.exit(1)
